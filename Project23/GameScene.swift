...
//  Project23
//
//  Created by Olibo moni on 24/02/2022.
//
...
class GameScene: SKScene {
    ...
    func endGame(triggeredByBomb: Bool){
        ...
        restart = SKLabelNode(text: "Pinch to Restart")
        restart.fontName = "Chalkduster"
        restart.fontColor = .red
        restart.fontSize = 45
        restart.zPosition = 1
        restart.position = CGPoint(x: gameOver.position.x , y: gameOver.position.y + 150)
        addChild(restart)
    ...
    func createEnemy(forceBomb: ForceBomb = .random){
        ...
        enemy.physicsBody = SKPhysicsBody(circleOfRadius: 64) // why 64
    ...
    func tossEnemies(){
        ...
        case .chain:
            createEnemy()
            DispatchQueue.main.asyncAfter(deadline: .now() + (chainDelay / 5.0 * 1)) { [weak self] in self?.createEnemy() }
            DispatchQueue.main.asyncAfter(deadline: .now() + (chainDelay / 5.0 * 2)) { [weak self] in self?.createEnemy() }
            DispatchQueue.main.asyncAfter(deadline: .now() + (chainDelay / 5.0 * 3)) { [weak self] in self?.createEnemy() }
            DispatchQueue.main.asyncAfter(deadline: .now() + (chainDelay / 5.0 * 4)) { [weak self] in self?.createEnemy() }
        case .fastChain:
            createEnemy()
            DispatchQueue.main.asyncAfter(deadline: .now() + (chainDelay / 10.0 * 1)) { [weak self] in self?.createEnemy() }
